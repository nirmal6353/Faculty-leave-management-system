...
else if(log_opt=='B')
{
	...
	while(fscanf(fp,"%s %s %c %d %s %s %s %c",Name,DEP,&oopt,&nn_days,FROM,TO,RESP,&STR)!=EOF)
	{
		...
		if(STR=='E')
		{
			...
			cout<<Name<<endl<<DEP<<endl<<FROM<<endl<<TO<<endl;
			cout<<endl<<"Do you want to take the responsibility?\n Y/N  ";
			char y_n;
			cin>>y_n;
...
else if(log_opt=='C')
{
	...
	char stt;	
	char fname[50];
	int ccl,sccl,vvl,ele;
	FILE *fp;
	char dirst2;
	fp=fopen("ADMIN.txt","r+");
	fseek(fp,1,SEEK_CUR);
	while(fscanf(fp,"%s %s %c %d %s %s %s %c %c",nname,depp,&optt,&no_days,frrom,too,rresp,&stt,&dirst2)!=EOF)
	{
		if(stt=='E')
		{
			FILE *fs;
			fs=fopen("datasheet.txt","r+");
			while(fscanf(fs,"%s %d %d %d %d",fname,&ccl,&sccl,&vvl,&ele)!=EOF)
			{
		...
		else if(dirst2=='Y')
		{
			...
			{
				char ffname[50];
				int clc,sclc,vlv,eel;
			}e;
			FILE *fs;
			fs=fopen("datasheet.txt","rb+");
			while(fread(&e,sizeof(e),1,fp)==1)
			{
				if(strcmp(e.ffname,nname)==0)
				{
					int len=strlen(nname);
			    	if(optt=='A')
					{
						e.clc=e.clc-no_days;
						fseek(fs,-(len+9),SEEK_CUR);
					...
					{
						fseek(fs,-(len+9),SEEK_CUR);
				        fprintf(fs,"%s %d %d %d",fname,ccl,sccl-no_days,vvl,ele);
					    fseek(fs,1,SEEK_CUR); 
				    }
				    if(optt=='C')
					{
						fseek(fs,-(len+9),SEEK_CUR);
				        fprintf(fs,"%s %d %d %d",fname,ccl,sccl,vvl-no_days,ele);
					    fseek(fs,1,SEEK_CUR);
				    }
				    if(optt=='D')
					{
						fseek(fs,-(len+9),SEEK_CUR);
				        fprintf(fs,"%s %d %d %d",fname,ccl,sccl,vvl,ele-no_days);
					    fseek(fs,1,SEEK_CUR);
				    }
			    }
		    }
